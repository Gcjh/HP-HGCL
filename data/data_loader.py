...
class data_loader:
    def __init__(self, path):
        ...
        self.links, self.degrees = self.load_links()

        degrees = {}
        for i in range(len(self.nodes['shift'])):
            if(i == len(self.nodes['shift']) - 1):
                break
            degree = []
            for j in range(self.nodes['shift'][i], self.nodes['shift'][i+1]):
                degree.append(self.degrees['all'][j])
            degrees[i] = np.array(degree)
            del degree
